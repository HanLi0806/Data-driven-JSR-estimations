...
for N in 1000:1000:numScen_budget 
    ...
    min_norm,output_norms_min = traj_norm_each_norm_min(state0_noise[:,1:N])
    expression_values_min= each_row_obsv_norm_min(output_norms_min, k, noise_bound_w, noise_bound_v)
    nonorms_min= noise_obsv_norm_min(output_norms_min, noise_bound_v, expression_values_min,k)
    spherenoise_min = sphere_noise_min(nonorms_min, noise_bound_w, noise_bound_v,k)
    ...
    local numTraj = size(state0_noise[:,1:N],2)                                                            

    ga_noise,P_noise = data_driven_lyapb_quad(state0_noise[:,1:N],state_noise[:,1:N];horizon=1,C=1e1,ub=10,lb=0,tol=1e-4,numIter=1e2,postprocess=true)
    ...
    cer = data_driven_jsr_sensitivityanalysis_quad_tuning_explicit(ga_sum,P_noise,dim,numTraj,k,numMode,horizon,min_norm,spherenoise_min,max_norm,spherenoise_max)

    push!(g_cer,cer)
    println("certificate_EX: $g_cer")
